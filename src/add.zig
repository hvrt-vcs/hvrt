...
pub const FileAdder = struct {
    ...
    pub fn addFiles(self: FileAdder, scratch: std.mem.Allocator, files: []const []const u8) !void {
        ...
        for (files) |file| {
            try self.addFile(scratch, tx, file, chunk_buffer, fifo_buf);
        }

        try tx.commit();
    }

    pub fn addFile(self: FileAdder, alloc: std.mem.Allocator, tx: sqlite.Transaction, file: []const u8, chunk_buffer: []u8, fifo_buf: []u8) !void {
        const file_sp = try tx.createSavepoint("add_single_file");
        errdefer file_sp.rollback() catch unreachable;
        ...
        const abs_repo_path = try self.repo_root.realpathAlloc(alloc, ".");
        const file_path_parts = [_][]const u8{ abs_repo_path, file };
        const abs_path = try std.fs.path.joinZ(alloc, &file_path_parts);
        defer alloc.free(abs_path);
