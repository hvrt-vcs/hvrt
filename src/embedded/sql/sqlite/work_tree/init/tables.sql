...
CREATE INDEX staged_to_add_blob_idx ON staged_to_add (
    blob_hash, blob_hash_algo
);
...
CREATE TABLE staged_to_add_parents (
    ...
    UNIQUE (path, "order"),
    FOREIGN KEY (path) REFERENCES staged_to_add (
        path
    ) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED
);
...
CREATE TABLE staged_to_remove (
    ...
    -- It is possible to both add and remove the file at the same path within
    -- the same commit. This would indicate that the file is being moved or
    ...
    -- However, if they removed the file, did `hvrt add .` then added a new
    -- file and again did `hvrt add .` it would see it as both a remove and an
    -- add and that might not be what the user wants; they could simply be
    -- trying to be thorough and be adding frequently as they get ready to
    -- commit, not thinking about the implications of what that means. This
    -- might require more thought as when digging into the implementation of
    -- this feature.
